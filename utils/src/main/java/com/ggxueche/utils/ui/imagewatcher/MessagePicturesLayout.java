package com.ggxueche.utils.ui.imagewatcher;
...
public class MessagePicturesLayout extends FrameLayout implements View.OnClickListener {
    ...
    private void notifyDataChanged() {
        ...
        if (urlListSize == 1 && VerifyUtil.isEmpty(thumbList.get(0))) {
            setVisibility(View.GONE);
        }
...
//                (int) ((getWidth() * 1f - mSpace * (column - 1)) / column);
        final int imageSize = urlListSize == 1 ? (int) ((getWidth() * 1f - mSpace*2) / 3) :
                (int) ((getWidth() * 1f - mSpace * (column - 1)) / column);
        ...
        for (int i = 0; i < iPictureList.size(); i++) {
            ...
            if (i < urlListSize) {
                ...
                GlideImgManager.getInstance().
                        loadImageView(getContext(),thumbList.get(i)+"@thumbs",R.mipmap.icon_empty_view,R.mipmap.icon_empty_view,iPicture);
                iPicture.setLayoutParams(lpChildImage);
                iPicture.setBackgroundResource(R.drawable.image_watcher_default_picture);
